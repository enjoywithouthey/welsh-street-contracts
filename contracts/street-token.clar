...
(define-fungible-token welsh-street-token)

;; Define errors
(define-constant ERR_NOT_CONTRACT_OWNER (err u900))
(define-constant ERR_NOT_TOKEN_OWNER (err u901))
(define-constant ERR_EXCEEDS_MAX_SUPPLY (err u902))
(define-constant ERR_TOKEN_MINT_NOT_A_BILLY (err u903))
...
(define-public (mint)
  (begin
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_NOT_CONTRACT_OWNER)
    (asserts! (<= (+ (ft-get-supply welsh-street-token) TOKEN_MINT) TOKEN_SUPPLY) ERR_EXCEEDS_MAX_SUPPLY)
    (ft-mint? welsh-street-token TOKEN_MINT tx-sender)
  )
)
...
(define-public (burn)
 (begin 
  (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_NOT_CONTRACT_OWNER)
   (ft-burn? welsh-street-token u1 tx-sender)
 )
)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? welsh-street-token amount sender recipient))
    (match memo
