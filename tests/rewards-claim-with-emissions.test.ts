...
describe("exchange initial liquidity", () => {
  ...
  if (disp) { 
    ...
    for (let i = 1; i <= EMISSION_COUNT; i++) {
        const emissionResult = simnet.callPublicFn(
            "street-token",
            "emission-mint",
            [],
            deployer
        ...
        if (emissionResult.result.type === "err") {
            expect(emissionResult.result).toEqual(Cl.error(Cl.uint(903)));
            if (disp) console.log(`emission-mint #${i + 1}: ERROR`, JSON.stringify(emissionResult.result, null, 2));
            break; 
        } else {
            ...
            expect(emissionResult.result).toEqual(
                Cl.ok(
                    Cl.tuple({
            ...
            if (disp) console.log(`emission-mint #${i + 1}:`, JSON.stringify(emissionResult.result, null, 2));
        }

        const updateRewardsResult = simnet.callPublicFn(
          "welsh-street-rewards",
          "update-rewards-b",
          [Cl.uint(EMISSION_AMOUNT)],
          deployer
        );
        expect(updateRewardsResult.result.type).toBe("ok");
        if (disp) console.log(`updated rewards b:  #${EMISSION_AMOUNT}`);
    ...
    if (disp) {
      const expectedEmissionRewards = EMISSION_COUNT * EMISSION_AMOUNT;
      const expectedTotalStreetRewards = bReward + bReward2 + bReward3 + bReward21 + expectedEmissionRewards;
      ...
      console.log("Street:", expectedTotalStreetRewards);
    }
    ...
    // STEP 11 - Check wallet2 balances after claiming rewards
    ...
    // STEP 12 - Check wallet2 reward balance after claiming rewards (should show zero unclaimed)
    console.log("STEP 12 - Check wallet2 reward balance AFTER claiming");
